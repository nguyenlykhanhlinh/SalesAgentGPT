...
class SalesGPTAPI:
    def __init__(
        ...
        model_name: str = "groq/llama3-8b-8192",  # Default model là Groq
        ...
        # Khởi tạo GroqCustomModel nếu model_name là Groq
        if "groq" in model_name:
            self.llm = GroqCustomModel(
                model=model_name,
            ...
            config.update(
                {
                    ...
                    "salesperson_name": config.get("salesperson_name", "Ted Lasso"),
                }
            )
    ...
    async def do(self, human_input=None):
        ...
        # Xử lý các bước trung gian nếu có sử dụng công cụ
        if self.use_tools and "intermediate_steps" in ai_log and ai_log["intermediate_steps"]:
            try:
